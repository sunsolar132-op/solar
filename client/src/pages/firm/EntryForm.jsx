...
export default function EntryForm({ type: initialType, isModal, onSuccess, onClose, defaultReturn = false }) {
  ...
  const [isReturn, setIsReturn] = useState(editEntry ? editEntry.type.endsWith('_RETURN') : defaultReturn);
  const type = isReturn ? `${initialType}_RETURN` : initialType;
  ...
  const [form, setForm] = useState({
    ...
    soId: editEntry?.soId || editEntry?.poId || ((initialType === 'SO' || initialType === 'BOOK') ? genSoId() : ''),
  ...
  const fetchDeliveryInfo = async (date) => {
    ...
    try {
      const info = await api.get(`/firm/delivery-info?date=${date}`);
  ...
  useEffect(() => {
    if (!isEditing) {
      if (initialType === 'SO' || initialType === 'BOOK') {
  ...
  const handleProductSelect = (index, id, name) => {
    ...
    // Auto-fill unit based on selected product
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (form.id) {
        await api.put(`/firm/entries/${form.id}`, data);
        addToast('Entry updated successfully!');
      } else {
        await api.post('/firm/entries', data);
        addToast(`${type.replace('_', ' ')} saved successfully!`);
      }
      ...
      if (isModal && onClose) onClose();
        ...
        setForm({
          ...
          remarkVersion: '', remark: '', deliveryDate: today,
          soId: (initialType === 'SO' || initialType === 'BOOK') ? genSoId() : '',
          billNo: ''
        });
  ...
  const formContent = (
    <div className="space-y-10">
      ...
      {!isModal && (
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="flex items-start gap-5">
            <div className={`rounded-full w-1.5 h-12 mt-1 ${isReturn ? 'bg-amber-500' : 'bg-blue-600'}`} />
            <div>
              <h1 className="text-4xl font-black text-slate-900 tracking-tighter">
                {isReturn ? `${title} Reset` : `${title} Registration`}
              </h1>
              <p className="text-xs font-bold text-slate-400 uppercase tracking-[0.2em] mt-2">
                New operational {initialType} ledger record
              </p>
            </div>
          </div>
          ...
          {(initialType === 'PURCHASE' || initialType === 'SALE') && (
            <div className="bg-white p-1.5 rounded-2xl shadow-sm border border-slate-100 flex gap-1">
              <button
                type="button"
                className={`px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${!isReturn ? 'bg-blue-600 text-white shadow-lg shadow-blue-100' : 'text-slate-400 hover:text-slate-600'}`}
              ...
              <button
                type="button"
                className={`px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${isReturn ? 'bg-amber-500 text-white shadow-lg shadow-amber-100' : 'text-slate-400 hover:text-slate-600'}`}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="md:col-span-2">
              <PartyDropdown
                ...
                category={initialType === 'PURCHASE' ? 'PURCHASE' : 'SALE'}
              />
            </div>

            {/* Row 3: Product Items Table */}
            ...
            {/* Row 4: ID / Delivery Date */}
            <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="group">
                <label className="field-label">{(initialType === 'SO' || initialType === 'BOOK') ? 'SO Serial' : 'Calculated Total Amount'}</label>
                <div className="input-field bg-slate-50 border-slate-200 text-slate-900 font-black flex items-center justify-between py-4 shadow-inner">
                  <span className="text-lg">{(initialType === 'SO' || initialType === 'BOOK') ? form.soId : `₹${totals.amount.toLocaleString()}`}</span>
                  {!isModal && <ArrowRight size={20} className="text-blue-500" />}
                </div>
              </div>
            ...
            <div className="md:col-span-2 group">
              ...
              <div className="relative">
                ...
                <textarea className="input-field pl-12 min-h-[100px] py-4" value={form.remark} onChange={e => setForm({ ...form, remark: e.target.value })} placeholder="Additional narration or remarks..." />
              </div>
            </div>
          </div>
              ...
              {loading ? (
                ...
                <>
                  ...
                  <span className="uppercase tracking-widest text-xs">
                    {form.id ? 'Update' : 'Execute'} {type.replace('_', ' ')}
                  </span>
                </>
              )}
            </button>
      ...
      {!isModal && (
        <div className="flex items-center gap-4 px-6 py-4 bg-blue-50/50 rounded-2xl border border-blue-100 text-blue-600 animate-in fade-in duration-700">
          <Info size={18} />
          <span className="text-xs font-black uppercase tracking-widest">Industrial logging active. All entries are tracked by unique transaction IDs.</span>
        </div>
      )}
    </div>
  );
